...
paramfile = r'expdisk_devauc_psf.sh'


def delete_files(filenames):
    """Delete files in a list if they exists."""
    for f in filenames:
        if os.path.isfile(f):
            os.remove(f)
...
def run_galfit(band, outdir, count, centroid_file):
    ...
    ingal = path + '/' + band + '_gal' + str(count) + '.fits'
    ...
    # object 3 is psf
    replace_galfit_param('1', centroid, object_num=3, fixed=False)
    replace_galfit_param('3', mag, object_num=3, fixed=False)
...
def main():
    ...
    for count in range(0, 302):
        run_galfit('f606w', galfit_outdir_f6, count, centroid_f6)

    # run galfit for f814w band
    for count in range(0, 302):
        run_galfit('f814w', galfit_outdir_f8, count, centroid_f8)

    # after running galfit on all galaxies delete temp files
